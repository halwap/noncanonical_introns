...
class Gene(GenomicSequence):
    ...
    def get_transcript_with_gaps_sequence(self, expanded=False):
        ...
        return sequence        
    def predict_introns(self):
        introns_to_assess, their_characteristics = [], []
        for intron in self.introns:
            #print(intron.prev_exon!=False, intron.next_exon!=False)
        ...
        if len(their_characteristics)==0: return
...
class Intron(GenomicSequence):
    ...
    def __init__(self, scaffold_name, scaffold_start, scaffold_end, sequence=None, strand=None, gene=None, support=None,
                 ...
                 test_score=0, gc_content=0.0, polipyrimidine_tract=0.0):
        GenomicSequence.__init__(self, scaffold_name, scaffold_start, scaffold_end, sequence=sequence, strand=strand)
        self.gene = gene
        ...
        self.polipyrimidine_tract = polipyrimidine_tract
        ...
        while True:
            ...
            if check == 'left':  # checking to the left
                ...
                new_variation = Intron(self.scaffold_name, scaffold_start=self.scaffold_start + i,
                                       scaffold_end=self.scaffold_end + i, gene=self.gene, sequence=new_seq,
                                       prev_exon=new_prev_exon, next_exon=new_next_exon)
            # TODO zadbać ładniej o to żeby się krótkie egzony nie robiły
    ...
    def check_ML_nonconv(self):
        if not self.prev_exon or not self.next_exon: return
        characteristics = compute_intron_characteristics(self.prev_exon.sequence[-10:], self.sequence, self.next_exon.sequence[:10]).reshape(1, -1)

        
        result = loaded_model.predict(characteristics)[0]
        proba = loaded_model.predict_proba(characteristics)[0][1]
    ...
    def add_test_annotation(self):
        ...
        #     self.test_global_annotation = 'intron_NK'
        # elif self.test_score_min < -5 and self.test_score_max > 5:
        #     if self.test_best_k_var.polipyrimidine_tract >= 0.6:
    ...
    def set_test_score(self):
        self.test_score = 0
        self.polipyrimidine_tract = calculate_pyrimidine_content(self.sequence[-12:-2])
        ...
        if self.canonical_borders:
            self.test_score += 10
        if self.polipyrimidine_tract >= 0.6:
            self.test_score += 3
